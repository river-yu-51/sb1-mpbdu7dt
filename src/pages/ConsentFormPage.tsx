...
const ConsentFormPage = () => {
  ...
  // View mode when route is /consent/view OR query param ?view=1
  const viewMode =
    location.pathname === "/consent/view" ||
    new URLSearchParams(location.search).get("view") === "1";

  // Routing rules
  if (viewMode && !user.consentSigned) return <Navigate to="/consent" replace />;
  if (!viewMode && user.consentSigned) return <Navigate to="/account" replace />;

  const fullName = `${user.firstName} ${user.lastName}`;

  const signedAtLabel = useMemo(() => {
    // If your AuthContext stores consent date under a different field name,
    // this safely falls back without crashing.
    const raw =
      (user as any).consentSignDate ||
      (user as any).consent_signed_at ||
      (user as any).consentSignedAt ||
      null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAgreed)
      return showNotification("You must agree to the terms to continue.", "error");

    if (signature.trim().toLowerCase() !== fullName.toLowerCase())
      return showNotification("Signature must match your full name.", "error");
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-lg p-8 md:p-12">
          ...
          <div className="prose prose-lg max-w-none mx-auto text-gray-700 space-y-6">
            ...
            <h3 className="!font-semibold !text-xl pt-6 border-t">
              Acknowledgment and Agreement
            </h3>
            <p className="text-base">
              By signing below, you confirm that you have read and understand the
              contents in this document. You also consent to participate in financial
              coaching sessions with me, Jacob Grima, with the understanding that you
              can discontinue our sessions at any time.
            </p>
          </div>

          {viewMode ? (
            <div className="mt-12 pt-8 border-t">
              <div className="bg-gray-50 border rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    {signedAtLabel ? ` on ${signedAtLabel}` : ""}.
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="px-4 py-2 rounded-lg border bg-white text-sm font-semibold hover:bg-gray-100"
                  >
                    Print / Save as PDF
                  </button>
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-grima-primary text-white text-sm font-semibold hover:bg-grima-dark"
                  >
                    Back to My Account
                  </button>
                </div>
            ...
            <form onSubmit={handleSubmit} className="mt-12 pt-8 border-t">
              ...
              <div className="mb-8">
                ...
                <input
                  ...
                  placeholder={`Type your full name to sign: ${fullName}`}
                />
              </div>
              ...
              <div className="mb-8">
                <label className="flex items-center">
                  <input
                    ...
                    className="h-5 w-5 rounded text-grima-primary focus:ring-grima-primary"
                  />
                  <span className="ml-3 text-gray-700">
                    I have read, understood, and agree to the terms outlined in this
                    consent form.
                  </span>
                </label>
              </div>
